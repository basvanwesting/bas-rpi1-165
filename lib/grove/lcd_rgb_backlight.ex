defmodule Grove.LCDRGBBacklight do
  @moduledoc """
  Conveniences for controlling a RGB LCD Backlight v2.0 Display.  The display should be connected
  to the I2C-1 port.

  Example usage:
  ```
  iex> {:ok, config} = Grove.LCDRGBBacklight.initialize()
  {:ok, %Grove.LCDRGBBacklight.Config{display_control: 12, entry_mode: 6, function: 56}}
  iex> {:ok, new_config} = Grove.LCDRGBBacklight.cursor_on(config)
  {:ok, %Grove.LCDRGBBacklight.Config{display_control: 14, entry_mode: 6, function: 56}}
  iex> Grove.LCDRGBBacklight.set_rgb(0, 255, 0)
  :ok
  iex> Grove.LCDRGBBacklight.set_text("hello world!")
  :ok
  ```
  """
  ...
  defmodule Config do
    @moduledoc """
    Module with struct to hold configuration
    """
  ...
  def autoscroll(%{entry_mode: entry_mode} = config) do
    ...
    {:ok, __MODULE__.Config.update_entry_mode(config, new_entry_mode)}
  end
  ...
  def cursor_blink_off(%{display_control: display_control} = config) do
    ...
    {:ok, __MODULE__.Config.update_display_control(config, new_display_control)}
  end
  ...
  @doc """
  Returns a Grove.LCDRGBBacklight.Config struct with default configuration.
  - 2 Line
  - 8 bit mode
  - 5x8 dots
  - display on
  ...
  def get_default_config() do
    ...
    %__MODULE__.Config{}
    |> __MODULE__.Config.update_function(function_config)
    |> __MODULE__.Config.update_display_control(display_control_config)
    |> __MODULE__.Config.update_entry_mode(entry_mode_config)
  end
  ...
  %Grove.LCDRGBBacklight.Config{} with initial configuration.
  """
  def initialize() do
    clear_display()
